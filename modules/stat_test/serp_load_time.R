...
ggsave("serp_loadtime_all.png", p, path = fig_path, units = "in", dpi = plot_resolution, height = fig_height, width = fig_width)
rm(p)
...
if (n_wiki > 1) {
  p <- serp_loadtime_function(by_wiki = TRUE) +
    ...
    wmf::theme_facet()
  ggsave("serp_loadtime_wiki.png", p, path = fig_path, units = "in", dpi = plot_resolution, height = 4 * n_wiki, width = fig_width)
  rm(p)
}

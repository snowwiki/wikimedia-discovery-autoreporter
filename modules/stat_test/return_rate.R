return_rate_function <- function(by_wiki = FALSE, ...) {
  returnRate_to_same_search <- events %>%
    ...
    group_by(!!! rlang::syms(c(switch(by_wiki, "wiki", NULL), "group", "search_id"))) %>%
    keep_where(sum(grepl("click", event)) > 0) %>% # Among search with at least 1 click
    arrange(group, search_id, timestamp) %>%
    ...
    summarize(return_to_same_search = sum(comeback), n_search = length(unique(search_id))) %>%
    group_by(!!! rlang::syms(c(switch(by_wiki, "wiki", NULL), "group"))) %>%
  ...
  returnRate_to_other_search <- events %>%
    ...
    summarize(another_search = length(unique(search_id)) > 1) %>% # comeback to make another search
    group_by(!!! rlang::syms(c(switch(by_wiki, "wiki", NULL), "group"))) %>%
...
ggsave("return_rate_all.png", p, path = fig_path, units = "in", dpi = plot_resolution, height = fig_height, width = fig_width, limitsize = FALSE)
rm(p)
...
if (n_wiki > 1) {
  p <- return_rate_function(by_wiki = TRUE) +
    ...
    wmf::theme_facet()
  ggsave("return_rate_wiki.png", p, path = fig_path, units = "in", dpi = plot_resolution, height = 3 * n_wiki, width = fig_width, limitsize = FALSE)
  rm(p)
}

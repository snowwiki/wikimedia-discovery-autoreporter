...
ggsave("zrr_all.png", p, path = fig_path, units = "in", dpi = plot_resolution, height = fig_height, width = fig_width)
rm(p)
...
if (n_wiki > 1) {
  p <- zrr_function(by_wiki = TRUE) +
    ...
    wmf::theme_facet()
  ggsave("zrr_wiki.png", p, path = fig_path, units = "in", dpi = plot_resolution, height = ifelse(n_wiki < 4, fig_height, 4 * ceiling(n_wiki / 3)), width = fig_width)
  rm(p)
}
...
p <- searches %>%
  ...
  wmf::theme_facet()
ggsave("daily_zrr.png", p, path = fig_path, units = "in", dpi = plot_resolution, height = ifelse(n_wiki < 3, fig_height, 3*ceiling(n_wiki/2)), width = fig_width)
rm(p)

if ("iwclick" %in% events$event) {
  ...
  ggsave("iwclick_position_all.png", p, path = fig_path, units = "in", dpi = plot_resolution, height = fig_height, width = fig_width)
  rm(p)
  ...
  if (iwclick_wiki > 1) {
    p <- iwclick_position_function(by_wiki = TRUE) +
      ...
      wmf::theme_facet()
    ggsave("iwclick_position_wiki.png", p, path = fig_path, units = "in", dpi = plot_resolution, height = 4 * iwclick_wiki, width = fig_width)
    rm(p)
  }

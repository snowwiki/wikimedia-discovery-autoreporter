...
p <- searches %>%
  ...
  wmf::theme_facet()
ggsave("daily_searches.png", p, path = fig_path, units = "in", dpi = plot_resolution, height = ifelse(n_wiki > 2, 2 * n_wiki, fig_height), width = fig_width)
rm(p)
...
ggsave("n_results_summary_all.png", p, path = fig_path, units = "in", dpi = plot_resolution, height = fig_height, width = fig_width)
rm(p)
...
if (n_wiki > 1) {
  ...
  ggsave("n_results_summary_wiki.png", p, path = fig_path, units = "in", dpi = plot_resolution, height = 5 * n_wiki, width = fig_width)
  rm(p)
}
...
if (exists("serp_offset")) {
  ...
  ggsave("serp_offset_summary_all.png", p, path = fig_path, units = "in", dpi = plot_resolution, height = fig_height, width = fig_width)
  rm(p)
  ...
  if (n_wiki > 1) {
    p <- serp_offset_summary_function(by_wiki = TRUE) +
      wmf::theme_facet() +
      ggplot2::facet_wrap(~ wiki, nrow = n_wiki, scales = "free")
    ggsave("serp_offset_summary_wiki.png", p, path = fig_path, units = "in", dpi = plot_resolution, height = 5 * n_wiki, width = fig_width)
    rm(p)
  }
}
